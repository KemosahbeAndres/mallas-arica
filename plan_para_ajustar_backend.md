# Plan de implementación — Mallas Arica sobre datos compartidos

> Documento de implementación. Destinatario: instancia de Claude con acceso al repositorio `mallas-arica` y al VPS.
> Objetivo: eliminar los contenedores MariaDB y Redis propios de Mallas Arica y consumir las instancias
...
> la red `backend-shared` existe y que `./scripts/provision-app.sh appmallas prod` ya se ejecutó y sus
...
- [ ] `./scripts/provision-app.sh appmallas prod` — pendiente, depende de lo anterior. Es el paso que
...
contenedores propios vía el `docker-compose.yml` de la raíz del repo (`mallas-arica-mariadb-dev`,
`mallas-arica-redis-dev`) — ese archivo **no se toca** en ninguna sección de este documento. Todas las
...
| Base de datos | `mallas_arica_prod` en contenedor propio | `appmallas_prod` en `shared-mariadb` |
| Usuario MariaDB | `mallas_arica_prod` | `appmallas`, con `GRANT` sobre `` `appmallas\_%`.* `` y `MAX_USER_CONNECTIONS 20` |
| Redis | Contenedor propio, sin auth | `shared-redis`, usuario ACL `appmallas`, claves `appmallas:*` |
...
| Backups | `backup-db.sh` propio de Mallas Arica | `/opt/infra/scripts/backup-app.sh appmallas` |
...
docker run --rm --entrypoint php ghcr.io/<owner>/mallas-arica:latest -m | grep -i redis
...
+# Aislamiento: base `appmallas_prod` + usuario `appmallas` (MariaDB); usuario ACL
+# `appmallas` limitado al patrón de claves `appmallas:*` (Redis).
...
+REDIS_USERNAME=appmallas
+REDIS_PASSWORD=
+# OBLIGATORIO. El ACL del servidor limita este usuario al patrón `appmallas:*`.
...
+REDIS_PREFIX=appmallas:
+REDIS_DB=1                      # sesiones
+REDIS_CACHE_DB=0                # caché
```

> `REDIS_PREFIX` y `CACHE_PREFIX` son cosas distintas y se **componen**: la clave real que llega a
> Redis es `appmallas:` + `CACHE_PREFIX` + clave. Por eso `~appmallas:*` cubre también la caché sin tocar
...
Ventana estimada: 3–8 minutos según el tamaño de `appmallas_prod`.
...
docker compose exec -T mariadb \
  mariadb-dump -uroot -p"${DB_ROOT_PASSWORD}" \
    ...
    appmallas_prod | gzip -9 > /root/appmallas_prod-premigracion-$(date +%F_%H%M).sql.gz

ls -lh /root/appmallas_prod-premigracion-*.sql.gz
gunzip -c /root/appmallas_prod-premigracion-*.sql.gz | head -5    # sanity check

# ---- 5.2 Congelar la aplicación (evita escrituras durante el traspaso) ----
docker compose stop app

# ---- 5.3 Restaurar en el MariaDB compartido ----
# La base appmallas_prod ya la creó provision-app.sh, vacía.
source /opt/infra/.env
gunzip -c /root/appmallas_prod-premigracion-*.sql.gz | \
  docker exec -i shared-mariadb \
    mariadb -uroot -p"${MARIADB_ROOT_PASSWORD}" appmallas_prod

# ---- 5.4 Verificar el traspaso ----
docker exec -i shared-mariadb mariadb -uroot -p"${MARIADB_ROOT_PASSWORD}" -e "
  SELECT TABLE_NAME, TABLE_ROWS
  FROM information_schema.TABLES
  WHERE TABLE_SCHEMA='appmallas_prod'
  ORDER BY TABLE_NAME;"
...
for t in cotizaciones cotizacion_items tarifas tipos_espacio tipos_malla tramos_altura; do
  o=$(docker compose exec -T mariadb mariadb -uroot -p"${DB_ROOT_PASSWORD}" -N -B \
        -e "SELECT COUNT(*) FROM appmallas_prod.$t" 2>/dev/null || echo "n/a")
...
docker exec -i shared-redis redis-cli --user backup --pass "${BACKUP_REDIS_PASSWORD}" \
  --no-auth-warning -n 0 --scan --pattern 'appmallas:*' | head
...
Conservar `/root/appmallas_prod-premigracion-*.sql.gz` **un mes como mínimo**. Es la única copia del
...
```diff
-- 3 contenedores por entorno, todos exclusivos de Mallas Arica (no compartidos con otros
...
+- Backups gestionados por infraestructura: `/opt/infra/scripts/backup-app.sh appmallas`
+  produce **un `.sql.gz` por base** (`appmallas\_%`) y **un `.jsonl.gz` por índice de Redis**
+  (export lógico del prefijo `appmallas:`), en `/var/backups/vps-apps/appmallas/`. Cron diario a
...
+  `vpsa-infra` (`/opt/infra/data/docker-compose.yml`). Mallas Arica solo aporta el contenedor
+  `mallas-arica-app`. Decisión tomada para alojar varias aplicaciones en 4 GB de RAM sin
+  duplicar motores de datos.
+- **Aislamiento por inquilino:** base `appmallas_prod` con `GRANT` sobre `` `appmallas\_%`.* ``
+  (el `\_` escapado es imprescindible: sin él `_` es comodín LIKE) y
+  `MAX_USER_CONNECTIONS 20`. En Redis, usuario ACL `appmallas` restringido al patrón
+  `~appmallas:*`, con `FLUSHALL` denegado y `FLUSHDB` permitido — este último lo necesita
+  `Illuminate\Cache\RedisStore::flush()`, que ignora el prefijo.
+- **`REDIS_PREFIX=appmallas:` es obligatorio en el `.env`.** Sin él, Laravel usa
...
+- **`DB_ROOT_PASSWORD` eliminada** del `.env` de Mallas Arica: la contraseña de root vive ahora
...
-                                    # servicios: app + mariadb + redis (todos propios de Mallas Arica)
...
más los scripts de provisión y backup por aplicación. Mallas Arica está registrada en `apps/appmallas.conf`.
...
- [ ] Credenciales de `provision-app.sh appmallas prod` guardadas
...
- [ ] Claves visibles bajo `appmallas:*` y en ningún otro prefijo
- [ ] `https://mallas.tinorte.cl/up` devuelve 200
- [ ] Cotizador completado a mano en el navegador (sesión + caché)
- [ ] `docker stats` muestra un único contenedor de Mallas Arica
- [ ] `/opt/infra/scripts/backup-app.sh appmallas` produce ambos artefactos
...
|---|---|---|
| `NOPERM ... no permissions to access one of the keys` | Falta `REDIS_PREFIX=appmallas:` o no coincide con el ACL | Añadirlo al `.env` y `config:cache` (el entrypoint lo hace al reiniciar) |
...
| `SQLSTATE[HY000] [1045] Access denied for user 'appmallas'` | El `.env` conserva `DB_USERNAME=appmallas_prod` | Cambiar a `appmallas` |
| `Access denied for user 'appmallas' to database 'X'` | La base no cae bajo `appmallas\_%` | Renombrarla con el prefijo o ampliar el GRANT |
...
- **El radio de daño creció.** Un OOM de `shared-mariadb` tumba todas las aplicaciones, no solo
  Mallas Arica. Los 2 GB de swap con `vm.swappiness=10` dan margen, pero conviene un monitor TCP en Uptime
  Kuma sobre `shared-mariadb:3306` y `shared-redis:6379`, no solo sobre `/up` de cada app.
- **El despliegue de Mallas Arica depende ahora de un repositorio externo.** El guard de §4 lo hace
...
- **La Etapa 2 no cambia.** Las fotos van a Cloudflare R2, no a MariaDB ni a Redis. El volumen
  `mallas-arica-storage` sigue siendo propio de Mallas Arica y sigue **sin estar en el backup automático**
  — es la deuda técnica pendiente de antes y este plan no la resuelve.
- **Si Mallas Arica suma clientes de pago**, se activan dos cosas ya documentadas: migrar a un VPS dedicado
