...
**Panel admin en subdominio propio (implementado):** el panel admin vive en `admin.{APP_DOMAIN}` (prod: `admin.mallasarica.cl`), separado del sitio público (`{APP_DOMAIN}` / `www.{APP_DOMAIN}`, ambos redirigiendo al canónico sin www) vía `Route::domain()` en `routes/web.php`. `config('app.domain')` (env `APP_DOMAIN`, sin protocolo ni subdominio) es la fuente de verdad — en local/CI vale `localhost` (admin → `admin.localhost`), en dev/staging `mallas.tinorte.cl`, en prod `mallasarica.cl`. Los nombres de ruta (`admin.*`, `home`) no cambiaron, solo el host que los sirve; `route('home')` y `route('admin.login')` siguen resolviendo la URL absoluta correcta automáticamente. Traefik (`deploy/docker-compose.yml`) enruta `admin.${APP_DOMAIN}` al mismo contenedor/servicio `mallas-arica`, sin credenciales ni proceso separado — es un solo Laravel, la separación es solo de host+rutas. Los tests HTTP a `/admin/*` usan el helper `TestCase::getAdmin()` (fija el host `admin.{APP_DOMAIN}`); los tests `Livewire::test(...)` no se ven afectados porque no pasan por el router HTTP.
...
>
> **Sección "Tipos de malla" (implementado, Sprint 7, pedido directo del dueño; precios quitados post-Sprint 13):** `resources/views/components/landing/mesh-types.blade.php`, ubicada entre "Qué protegemos" y "Cómo trabajamos". Muestra **3 espesores**, todos solo con el peso que soportan, **sin precio** — el valor se cotiza siempre en la visita técnica (0,80 mm rombo 5×5 cm, soporta 200 kg/m²; 0,90 mm rombo 3×3 cm para gatitos pequeños, soporta 250 kg/m²; 1,9 mm rombo 4×4 cm para gatitos mordedores, soporta 300 kg/m²) — **contenido estático hardcodeado en el Blade, no consulta el modelo `TipoMalla`** (que solo tiene 2 tipos por uso: Estándar/Reforzada mascotas, usados por el cálculo de tarifas ya oculto al público). La sección separa visualmente la **malla** (grid de 3 espesores) del **sistema de anclaje** (bloque aparte, con su propio encabezado "Sistema de anclaje"): ángulos de aluminio 20×20×1,2 mm con amarres de alambre galvanizado (estándar), y **Sistema Netzen** con arpones de poliamida, "a pedido", explícitamente descrito como **certificado internacionalmente** (badge rojo).
>
