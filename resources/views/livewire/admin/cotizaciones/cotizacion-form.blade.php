...
<form wire:submit="guardar" class="flex flex-col gap-6">
                ...
                <div class="mt-2 flex flex-wrap gap-2">
                    @foreach ($this->clientesFiltrados as $c)
                        <button type="button" wire:click="$set('clienteId', {{ $c->id }})"
                            class="rounded-lg px-3 py-1.5 text-sm transition-colors {{ $clienteId === $c->id ? 'bg-brand-red-ui text-white' : 'border-line bg-white text-ink-soft hover:bg-cream-deep border' }}">
                            {{ $c->nombre }}
                        </button>
                    @endforeach
    ...
    <section class="border-line rounded-2xl border bg-white p-6">
        ...
        <div class="mt-4 flex flex-col gap-2">
            ...
            @foreach ($items as $i => $item)
                <div class="grid grid-cols-2 items-center gap-2 sm:grid-cols-[1fr_7rem_5rem_5rem_7rem_2rem]" wire:key="item-{{ $i }}">
                    ...
                    <input type="number" min="0" step="0.01" wire:model.live.debounce.400ms="items.{{ $i }}.cantidad"
                        class="border-line focus:border-brand-red-ui w-full rounded-lg border px-2 py-2 text-right text-sm">
        ...
        <div class="border-line mt-6 flex flex-col items-end gap-1 border-t pt-4 text-sm">
            ...
            <div class="flex w-full max-w-xs items-center justify-between border-t border-line pt-2">
