<div class="grid gap-6 lg:grid-cols-[20rem_1fr]">
    ...
    <div class="flex flex-col gap-4">
        ...
        <div class="border-line divide-line divide-y overflow-hidden rounded-xl border bg-white">
            @forelse ($this->clientes as $cliente)
                <button
                    type="button"
                    wire:key="cliente-{{ $cliente->id }}"
                    wire:click="seleccionar({{ $cliente->id }})"
                    class="flex w-full items-start justify-between gap-3 px-4 py-3 text-left transition-colors {{ $seleccionado === $cliente->id ? 'bg-cream-deep' : 'hover:bg-cream-deep/60' }}"
                >
                    <span>
            ...
            <div class="border-line border-t pt-5">
                ...
                <div class="mt-4 flex flex-col gap-3">
                    @forelse ($direcciones as $indice => $dir)
                        <div class="border-line bg-cream-deep/40 flex items-start gap-2 rounded-xl border p-3" wire:key="dir-{{ $indice }}">
